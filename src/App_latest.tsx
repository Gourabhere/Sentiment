...
console.log(analyzeSentiment_natural('whatwentwell') || analyzeSentiment_natural('whatDidNotGoWell'));
...
function App() {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    reader.onload = (e) => {
      ...
      const transformedData: SentimentData[] = jsonData.map((row: any) => ({
        ...
        whatwentWellScore: analyzeSentiment_natural(row.What_went_well),
        whatDidNotGowellScore: analyzeSentiment_natural(row.what_did_not_go_well),
      }));
  ...
  const handleFilterChange = (filters: {
    ...
    if (filters.sprint) {
      filtered = filtered.filter((item) => filters.sprint.includes(item.sprint));
    }
    ...
    if (filters.sentiment.length > 0) {
      filtered = filtered.filter((item) => {
        const sentimentCategory =
          item.sentimentScore > 0 ? 'positive' : item.sentimentScore < 0 ? 'negative' : 'neutral';
  ...
  return (
    <div className={`min-h-screen bg-gray-50 dark:bg-gray-900 ${isDarkMode ? 'dark' : ''}`}>
      <Header isDarkMode={isDarkMode} toggleDarkMode={toggleDarkMode} onFileUpload={handleFileUpload} />
